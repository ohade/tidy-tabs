...
function normalizeCategoryPlan(rawCategories) {
  ...
  for (const rawCategory of rawCategories) {
    ...
    if (!name || names.has(key) || !CODEX_COLORS.includes(rawCategory.color)) continue;
    names.add(key);
    categories.push({ name, color: rawCategory.color });
  }
  ...
  if (tabsToClassify.length > 0) {
    if (tabsToClassify.length <= CODEX_BATCH_SIZE) {
      ...
      for (const category of planned.categories) {
        const bucket = categoryBuckets.get(category.name.toLowerCase());
        for (let offset = 0; offset < bucket.tab_ids.length; offset += MAX_TABS_PER_GROUP) {
          const chunkNumber = Math.floor(offset / MAX_TABS_PER_GROUP) + 1;
          groups.push({
            name: chunkNumber === 1 ? category.name : `${category.name} ${chunkNumber}`,
            color: category.color,
            tab_ids: bucket.tab_ids.slice(offset, offset + MAX_TABS_PER_GROUP)
          });
        }
      }
    }
